...
export class LCDQueryClient {
  ...
  async pythPriceStates(_params: QueryPythPriceStatesRequest = {}): Promise<QueryPythPriceStatesResponseSDKType> {
    const endpoint = `injective/oracle/v1beta1/pyth_price_states`;
    return await this.req.get<QueryPythPriceStatesResponseSDKType>(endpoint);
  }
  ...
  /* OraclePrice */
  async oraclePrice(params: QueryOraclePriceRequest): Promise<QueryOraclePriceResponseSDKType> {
    const options: any = {
      params: {}
    };
    if (typeof params?.oracleType !== "undefined") {
      options.params.oracle_type = params.oracleType;
    }
    ...
    const endpoint = `injective/oracle/v1beta1/price`;
    return await this.req.get<QueryOraclePriceResponseSDKType>(endpoint, options);
  }
  /* PythPrice */
  async pythPrice(params: QueryPythPriceRequest): Promise<QueryPythPriceResponseSDKType> {
    const options: any = {
      params: {}
    };
    if (typeof params?.priceId !== "undefined") {
      options.params.price_id = params.priceId;
    }
    const endpoint = `injective/oracle/v1beta1/pyth_price`;
    return await this.req.get<QueryPythPriceResponseSDKType>(endpoint, options);
  }
}
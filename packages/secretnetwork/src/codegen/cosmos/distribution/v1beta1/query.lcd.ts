...
export class LCDQueryClient {
  ...
  /* DelegatorWithdrawAddress queries withdraw address of a delegator. */
  async foundationTax(_params: QueryFoundationTaxRequest = {}): Promise<QueryFoundationTaxResponseSDKType> {
    const endpoint = `cosmos/distribution/v1beta1/foundation_tax`;
    return await this.req.get<QueryFoundationTaxResponseSDKType>(endpoint);
  }
  /* RestakeThreshold queries the community pool coins. */
  async restakeThreshold(_params: QueryRestakeThresholdRequest = {}): Promise<QueryRestakeThresholdResponseSDKType> {
    const endpoint = `cosmos/distribution/v1beta1/restake_threshold`;
    return await this.req.get<QueryRestakeThresholdResponseSDKType>(endpoint);
  }
  /* RestakeThreshold queries the community pool coins. */
  async restakingEntries(params: QueryRestakeEntriesRequest): Promise<QueryRestakingEntriesResponseSDKType> {
    const options: any = {
      params: {}
    };
    if (typeof params?.delegator !== "undefined") {
      options.params.delegator = params.delegator;
    }
    const endpoint = `cosmos/distribution/v1beta1/restake_entries`;
    return await this.req.get<QueryRestakingEntriesResponseSDKType>(endpoint, options);
  }
}